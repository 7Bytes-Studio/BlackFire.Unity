...
namespace BlackFireFramework.Unity
{
	...
	public sealed partial class GameManager
    {
        ...
        private void InitProcess()
        {
            ...
            CheckAndAddProcess(Utility.Type.GetTypeInProject(m_FirstProcess),m_FirstProcess);


            var tps = Utility.Type.GetTypesInProject(m_AvailableProcesses);
            for (int i = 0; i < tps.Length; i++)
            {
                if (tps[i].FullName != m_FirstProcess)
                {
                    CheckAndAddProcess(tps[i],tps[i].FullName);
                }
            }
