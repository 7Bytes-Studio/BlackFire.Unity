...
namespace BlackFire.Unity.Editor {
	...
	public class ReorderableListControl {
		...
		private void DrawListItem(Rect position, IReorderableListAdaptor adaptor, int itemIndex) {
			bool isRepainting = UnityEngine.Event.current.type == EventType.Repaint;
			...
			try {
				...
				if ((Flags & ReorderableListFlags.DisableContextMenu) == 0) {
					if (UnityEngine.Event.current.GetTypeForControl(_controlID) == EventType.ContextClick && position.Contains(UnityEngine.Event.current.mousePosition)) {
		...
		private void DrawListContainerAndItems(Rect position, IReorderableListAdaptor adaptor) {
			...
			EventType eventType = UnityEngine.Event.current.GetTypeForControl(_controlID);
			...
			switch (eventType) {
				...
				case EventType.KeyDown:
					if (_tracking && UnityEngine.Event.current.keyCode == KeyCode.Escape) {
						StopTrackingReorderDrag();
						UnityEngine.Event.current.Use();
					}
					break;
					...
					switch (eventType) {
						case EventType.MouseDown:
							if (GUI.enabled && itemPosition.Contains(mousePosition)) {
								...
								if (_allowReordering && adaptor.CanDrag(i) && UnityEngine.Event.current.button == 0) {
...
//*/
				...
				if (s_DropTargetNestedCounter == initialDropTargetNestedCounterValue) {
					if (UnityEngine.Event.current.mousePosition.y >= lastMidPoint) {
		...
		private void HandleDropInsertion(Rect position, IReorderableListAdaptor adaptor) {
			...
			if (target.CanDropInsert(_insertionIndex)) {
				...
				if (DragAndDrop.activeControlID == _controlID && UnityEngine.Event.current.type == EventType.Repaint)
