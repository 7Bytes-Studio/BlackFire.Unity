...
namespace BlackFire.Unity.Editor {
	...
	public class SerializedPropertyAdaptor : IReorderableListAdaptor {
		...
		public virtual void DrawItem(Rect position, int index) {
            //modify by Thibault to support custom class and struct
			//EditorGUI.PropertyField(position, this[index], GUIContent.none, false);

            GUIContent content;

            if (_arrayProperty.GetArrayElementAtIndex(index).propertyType == SerializedPropertyType.Generic)
            {
#if UNITY_4_5
                content = new GUIContent("Element " + index.ToString());
#else
                content = new GUIContent(_arrayProperty.GetArrayElementAtIndex(index).displayName);
#endif
            }
            else
            {
            ...
            EditorGUI.PropertyField(position, this[index], 
                                    content, 
                                    true);
		}
