		...
		public static bool IconButton(Rect position, bool visible, Texture2D iconNormal, Texture2D iconActive, GUIStyle style) {
			...
			switch (UnityEngine.Event.current.GetTypeForControl(controlID)) {
				case EventType.MouseDown:
					...
					if (GUI.enabled && UnityEngine.Event.current.button != 1 && position.Contains(UnityEngine.Event.current.mousePosition)) {
				...
				case EventType.MouseUp:
					if (GUIUtility.hotControl == controlID) {
						...
						result = position.Contains(UnityEngine.Event.current.mousePosition);
						UnityEngine.Event.current.Use();
					}
					break;
				...
				case EventType.Repaint:
					if (visible) {
						bool isActive = GUIUtility.hotControl == controlID && position.Contains(UnityEngine.Event.current.mousePosition);
